...
public static class LinqFuncToSqlLangHelper
{
    ...
    private static readonly Dictionary<string, string> _keyDictionary = new()
    {
        [LinqAndAlso] = SqlAnd,
        [LinqOrElse] = SqlOr,
    ...
    private static readonly Dictionary<string, string> _opDictionary = new()
    {
        [LinqSlash] = SqlSlash,
        [LinqQuote] = SqlQuote,
        [LinqEqual] = SqlEqual,
    ...
    {
        _keyDictionary.TryGetValue(key, out var sqlFunc);
        return sqlFunc;
    }

    public static string GetOperator(string key)
    {
        _opDictionary.TryGetValue(key, out var sqlOperator);
        return sqlOperator;
    }

    public static string ReplaceOperators(this string linqExpression)
    {
        foreach (var op in _opDictionary)
        {
